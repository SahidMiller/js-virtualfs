...
type mkdirOptions = | string | { recursive?: boolean, mode?: number };
...
class VirtualFS {
  ...
  chownr (path: path, uid: number, gid: number, callback: callback = callbackUp): void {
    this._callAsync(
      this.chownrSync.bind(this),
      [path, uid, gid],
      callback,
      callback
    );
  ...
  chownrSync (path: path, uid: number, gid: number): void {
    path = this._getPath(path)
  ...
  mkdirSync(path: path, options: mkdirOptions): void {
